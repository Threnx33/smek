...
const certificateTemplatesFilterSchema = z.object({
...
const defaultFilterValues: Partial<CertificateTemplatesFilterSchema> = {
  templateStatus: [],
  templateVisibility: [],
};

type CertificateTemplatesFilterSchema = z.infer<
  typeof certificateTemplatesFilterSchema
>;

interface CertificateTemplatesFilterProps<TData> {
  table: ReactTable<TData>;
  className?: string;
}

export function CertificatesTemplatesFilterForm<TData>({
  table,
  className,
}: CertificateTemplatesFilterProps<TData>) {
  const form = useForm<CertificateTemplatesFilterSchema>({
    resolver: zodResolver(certificateTemplatesFilterSchema),
    defaultValues: defaultFilterValues,
  });

  const closeRef = useRef<HTMLButtonElement>(null);

  async function onSubmit(data: CertificateTemplatesFilterSchema) {
